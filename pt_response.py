import ROOT
import argparse
import numpy as np

parser = argparse.ArgumentParser()
parser.add_argument('--type')
parser.add_argument("--batch", action = 'store_true')
parser.add_argument("--merge", action = 'store_true')

args = parser.parse_args()
sample = args.type
batch = args.batch
merge = args.merge

ntuple = 'gentau' if sample == 'TGUN' else 'jet' if sample == 'FAKES' else ''

ROOT.gROOT.SetBatch(batch)
ROOT.gStyle.SetOptStat(0)

file_stdr = ROOT.TFile.Open('tau_{}_tuple_{}_standard.root'.format(ntuple, sample), 'READ')
file_0p90 = ROOT.TFile.Open('tau_{}_tuple_{}_0p90eff.root' .format(ntuple, sample), 'READ')
file_0p05 = ROOT.TFile.Open('tau_{}_tuple_{}_0p95eff.root' .format(ntuple, sample), 'READ')

histo_stdr = ROOT.TH1F('histo_stdr', '', 100, -1, 5)
histo_0p90 = ROOT.TH1F('histo_0p90', '', 100, -1, 5)
histo_0p95 = ROOT.TH1F('histo_0p95', '', 100, -1, 5)

histo_stdr.GetXaxis().SetTitle('1. - RECO_pT / GEN_vis_pT') ; histo_stdr.SetTitle('stdr')
histo_0p90.GetXaxis().SetTitle('1. - RECO_pT / GEN_vis_pT') ; histo_0p90.SetTitle('0p90')
histo_0p95.GetXaxis().SetTitle('1. - RECO_pT / GEN_vis_pT') ; histo_0p95.SetTitle('0p95')

tree_stdr = file_stdr.Get('tree')
tree_0p90 = file_0p90.Get('tree')
tree_0p95 = file_0p05.Get('tree')

files_to_merge = [] 

base_selection = ' && '.join(['tau_gen_vis_pt > 20', 'abs(tau_gen_vis_eta) < 2.3'])
for dm in [0, 1, 2, 3, 4, 10, 11, 12, 13, 14]:
    selection = ' && '.join([base_selection, 'tau_gen_decaymode == {}'.format(dm)])
    tree_stdr.Draw('1. - (tau_reco_pt / tau_gen_vis_pt) >> histo_stdr', selection)
    tree_0p90.Draw('1. - (tau_reco_pt / tau_gen_vis_pt) >> histo_0p90', selection)
    tree_0p95.Draw('1. - (tau_reco_pt / tau_gen_vis_pt) >> histo_0p95', selection)

    can = ROOT.TCanvas()
    can.Divide(2, 2)

    histo_stdr.SetTitle('stdr_{}_DM'.format(dm))
    histo_0p90.SetTitle('0p90_{}_DM'.format(dm))
    histo_0p95.SetTitle('0p95_{}_DM'.format(dm))

    can.cd(1) ; histo_stdr.Draw('HIST')
    can.cd(2) ; histo_0p90.Draw('HIST')
    can.cd(3) ; histo_0p95.Draw('HIST')

    file_name = 'pdf/{}/pt_response_DM{}.pdf'.format(sample, dm)
    can.SaveAs(file_name, 'pdf')
    files_to_merge.append(file_name)
    
if merge:
	import os
	from PyPDF2 import PdfFileMerger
	
	merger = PdfFileMerger()
	
	for pdf in files_to_merge:
		if not os.path.isfile(pdf): continue
		merger.append(pdf)
	
	merger.write('pdf/{}/pt_response.pdf'.format(sample))
	merger.close()

	print 'INFO: merged', len(files_to_merge), 'pdf files'
	
	for pdf in files_to_merge:
		os.remove(pdf)
	
	print 'INFO: single pdf files deleted'
